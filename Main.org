  ...
  dashboardBody(
    tabItems(
      tabItem(
        tabName = "T1",
        fluidRow(
            box(width = 2, # taille de la box
                column(
                    width = 12, #taille ?
                    selectInput("i_sp", 
                                label = "choose ?",
                                choices = 1:length(breweries91), 
                            selected = 1), 
                  # textInput("village", value = breweries91$village[1]), 
                    selectInput("irow", #input$irow
                                label = "irow ?",
                                choices = 1:2, 
                                selected = 1)
                )
...
server <- function(input, output, session) {
  ...
  points <- eventReactive(input$recalc, {
    cbind(sample(460: 500/ 100, 10) , sample(4700: 4730/ 100, 10))
  }, ignoreNULL = FALSE)
