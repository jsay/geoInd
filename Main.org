...
*** The Informational Content of Geographical Indications
    ...
    to compute a continuous quality score for each of the 60000
    parcels of the area.  The Figure below reports the score
    distribution according to actual classification, with and without
    the correction for unequal treatments between administrative
    /communes/.
...
modélisation de la hiérarchie des AOC qui permet de noter sur 100
...
#+end_example
    ...
    crus (=table(Geo.Cad$AOC, Geo.Cad$AOCtp)=).  Pour les régionaux,
    il s'agit d'avoir les Bourgognes Côte d'Or et les hautes côtes et
    Notre Dame La Chapelle (on n'a pas Le Chapitre).
...
Dans le
    Data paper, il s'agira d'identifier les grands crus des villages
    avec et sans nom reconnus pour retrouver la structure
    hiérarchique.
...
Poly.tst$PrdRaw <- aggregate(Geo.Aoc$U900raw,
                             by= list(Geo.Aoc$Concat), mean)[, 2]
Poly.tst$PrdCor <- aggregate(Geo.Aoc$U900cor,
                             by= list(Geo.Aoc$Concat), mean)[, 2]
...
#+LATEX: \vspace*{-1cm} \footnotesize \input{Tables/StatDes.tex}
...
*** Resume
    ...
    Two =dimensions of differentiation=:
      + Vertical: BGOR< BOUR< VILL< PRCU< GCRU
      + Horizontal: /Communes/ ~ /Lieux dits/ ~ /Parcelles/

#+LATEX: \vspace{.5cm}\pause

    =Complex interaction= between the two:
      + /commune/ and /village/ synonyms (/Côte de Beaune-Village/)
      + some /communes/ share the name of /grand cru/
      + some /communes/ share a same /grand cru/
      + some /communes/ share the name of the same /grand cru/
...
#+LATEX: \vspace{.5cm}
     ...
     =So= we fit Ordered Generalized Additive Models
...
\begin{flalign*}
  \mathbb{V}\left[q\left(X^*\right)\right]=
    ...
    \underbrace{\mathbb{E}\left\{\mathbb{V}\left[q\left(X^*\right)\mid y, c\right]\right\}}_{\mbox{joint residual}}&& [D2]
\end{flalign*}\pause
#+end_export
...
#+LaTeX: \Large
    ...
    Open data, \textbf{R} codes and Shiny app : @@latex:
