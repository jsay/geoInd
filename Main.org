...
*** The Informational Content of Geographical Indications
    ...
    Predicting vineyard quality from econometric modeling of
    geographical indications in Côte d'Or (Burgundy, France)

    Prédire la qualité du terroir à partir d'une modélisation
    économetrique des appellation d'origine en Côte d'Or (Bourgogne,
    France)
...
cc <- rbind(
    data.frame(AOC= revalue(factor(Geo.Plt$AOC), revAOC),
               ...
               Prediction= "Uncorrected: with communes fixed effects"),
    data.frame(AOC= revalue(factor(Geo.Plt$AOC), revAOC),
               Score= Geo.Plt$G900cor,
               Prediction= "Corrected: without communes fixed effects"))

ggplot(cc, aes(factor(AOC), Score, fill= Prediction))+
    geom_split_violin()+
