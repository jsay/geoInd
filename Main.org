...
*** The Informational Content of Geographical Indications
    ...
    R code and Shiny application used to predict vineyard quality in
    the /Côte de Beaune/ and /Côte de Nuits/ (Burgundy, France).
    Relating fine-scale biophysical attributes of vineyards to the
    hierarchical scheme of /Appellations d'Origine Contrôlée/ allows
    to estimate a continuous quality score for each of the 60000
    plots.  The Figure below reports the score distribution for each
    actual item, with and without the correction for unequal
    treatments between administrative /communes/.
...
pour étudier statistiquement les relations entre les caractéristiques
biophysiques (topographie, géologie, pédologie) des parcelles
viticoles et les appellations d'origine contrôlée (AOC).  La zone
d'étude comprend les 31 communes de la Côte d'Or qui forment les Côtes
de Beaune et de Nuits.  L'intérêt de ces données est illustré par une
modélisation de la hiérarchie des AOC qui permet de classer en continu
chaque parcelle de vigne en accord avec les principes historiques de
désignation.  Les données et prédictions du modèle sont disponibles
sous licence Creative Commons XX sur https://data.inra.fr/geoInd et
sont consultables par le biais d'une application Shiny sur
[[http://github.com/jsay/geoInd/]].\\
...
*** Du cadastre aux AOC
    ...
    dans la Fig:1.  La géométrie du parcellaires est issue de la BD
    parcellaire de l'IGN version X.XX téléchargée le XX/XX/2018.  Deux
    traitements ont été effectués au préalable, nous avons calculé à
    l'aide d'un système d'information géographique (SIG) des
    caractéristiques géométriques des parcelles (surface, périmètre,
    et distance maximale entre deux sommets, voir Table [[Tab:1]]) et
    appariée l'information sur les AOC à partir du fichier de l'INAO
    sur \url{http://data.gouv.fr/inao}.  Sur ce deuxième point, nous avons retenu
    pour chaque parcelle l'AOC la plus haute dans la hiérarchie.
    Nous avons vérifiée la distribution des surface par AOC par
    commune, colle assez bien avec le bouquin.
...
#+end_example
    ...
    La base parcellaire contient donc $110\,350$ observations et 16
    variables en incluant =PAR2RAS= un identifiant pour l'appariement
    avec les données raster.  L'information sur les AOC tient en 3
    variables, =AOC=, =AOCtp= et =AOClb=.  Les valeurs omises
    correspondent à l'absence d'AOC.  Attention ce ne sont pas des
    AOC, en particulier pour les Bourgognes régionaux et les premiers
    crus où on utilise les dénominations (=table(Geo.Cad$AOC,
    Geo.Cad$AOCtp)=).  Le nom générique pourrait être les indications
    géographiques.  Pour les régionaux, il s'agit d'avoir les
    Bourgognes Côte d'Or et les hautes côtes et Notre Dame La Chapelle
    (on n'a pas Le Chapitre).
...
(labs <- data.frame(NOM= c("IDU", "CODECOM", "AREA", "PERIM", "MAXDIST",
                    ...
                    TYPE= c("/Caractère issu de concaténation/",
                            "/Caractère de taille 5/",
                            "/Numérique en mètres carrés/",
                            "/Numérique en mètres/", 1: 12),
                    DESCRIPTION=
                        c("Identifiant cadastral", "Code INSEE commune",
                          ...
                          "Grand Cru", "Rang dans la hiérarchie",
                          "Type de nom", "Libellé")))
#+end_src
...
|---------+-----------------------------------+------------------------------------------|
| IDU     | /Caractère issu de concaténation/ | Identifiant cadastral                    |
| CODECOM |           /Caractère de taille 5/ | Code INSEE commune                       |
| AREA    |      /Numérique en mètres carrés/ | Surface de la parcelle                   |
| PERIM   |             /Numérique en mètres/ | Périmètre de la parcelle                 |
| MAXDIST |                                 1 | Distance maximale entre deux sommets     |
| PAR2RAS |                                 2 | Identifiant pour appariement avec raster |
| PAOC    |                                 3 | Hors périmètre AOC                       |
| BGOR    |                                 4 | Coteaux Bourguignon                      |
| BOUR    |                                 5 | Bourgogne Régional                       |
| VILL    |                                 6 | Bourgogne Village                        |
| COMM    |                                 7 | Bourgogne Communal                       |
| PCRU    |                                 8 | Premier Cru                              |
| GCRU    |                                 9 | Grand Cru                                |
| AOC     |                                10 | Rang dans la hiérarchie                  |
| AOCtp   |                                11 | Type de nom                              |
| AOClb   |                                12 | Libellé                                  |

*** Enrichissement de la topographie

    Les données raster sont également issues du cloud, avec le fichier
    =vitidem.csv= qui contient 15 millions de cellules pour 13
    variables.  Nous transformons la variable catégorielle =MOS= sur
    ...
    quantitatives seront simplement moyennées au niveau des parcelles.
    Nous pourrions imaginer d'autres méthodes d'agrégation et reporter
    d'autres statistiques: pour plus tard.
...
#+end_example
    ...
    s'apparie à aucune des cellules raster.  Après une identification
    de ces petites parcelles (taille médiane de 10 m$^2$), il s'agit
    de spécificités du cadastre que nous choisissons de négliger.
    Cela revient à enlever 2.7 ha, moins de 0.01 % de la surface
    totale.
...
(labt <- data.frame(NOM= c("XL93", "YL93", "NOMOS", "URBAN", "FOREST",
                           ...
                           "PERMEA"),
                    TYPE= c("/Numérique/", "/Numérique/", 1: 9),
                    DESCRIPTION= c("Latitude Lambert 93",
                                   "Longitude Lambert 93",
                                   "Part hors MOS",
                                   "Part en usage urbain",
                                   "Part en Forêt", "Part en eau",
...
| NOM    |        TYPE | DESCRIPTION                 |
|--------+-------------+-----------------------------|
| XL93   | /Numérique/ | Latitude Lambert 93         |
| YL93   | /Numérique/ | Longitude Lambert 93        |
| NOMOS  |           1 | Part hors MOS               |
| URBAN  |           2 | Part en usage urbain        |
| FOREST |           3 | Part en Forêt               |
| WATER  |           4 | Part en eau                 |
| DEM    |           5 | Altitude moyenne en mètres  |
| SLOPE  |           6 | Pente moyenne en degrés     |
| ASPECT |           7 | Exposition (degrés)         |
| SOLAR  |           8 | Radiation solaire en Joules |
| PERMEA |           9 | Perméabilité                |
...
*** Enrichissement de la géologie
    ...
    structuraux et divers.  Nous utilisons simplement la couche
    principale XX de laquelle nous extrayons les variables XX.  On a
    fait une sélection sur les valeurs omises et sur la redondance
    d'information.
...
#+end_example
    ...
    Le code précédent permet d'apparier les 14000 polygones avec les
    13 variables sur la géologie (à l'échelle de la Côte d'Or).  J'ai
    fait une demande et une relance au BRGM pour obtenir le
    dictionnaire des variables reportées dans le tableau ci-dessous,
    qui est incomplet car je n'ai pas encore eu de réponse.  Les
    valeurs manquantes sont distinctes des valeurs manquantes de la
    topographie.

#+begin_src R :exports results :results value :colnames yes :rownames no
(labu <- data.frame(NOM= c("CODE", "NOTATION", "DESCR", "TYPEGEOL",
                           "APLOCALE", "TYPEAP", "GEOLNAT", "ISOPIQUE",
                           "AGEDEB", "ERADEB", "SYSDEB", "LITHOLOGIE", 
                           "DURETE", "ENVIRONMT","GEOCHIMIE", "LITHOCOM"),
                    VALEURS= c("/Caractère, 31 modalités/",
                               "/Caractère, 31 modalités/",
                               "/Caractère, 31 modalités/",
                               "/Caractère, 4 modalités/",
                               "/Caractère, 28 modalités/",
                               "/Caractère, 7 modalités/",
                               "/Caractère, 3 modalités/",
                               "/Caractère, 4 modalités/",
                               "/Caractère, 24 modalités/",
                               "/Caractère, 2 modalités/",
                               "/Caractère, 5 modalités/",
                               "/Caractère, 16 modalités/",
                               "/Caractère, 3 modalités/",
                               "/Caractère, 9 modalités/",
                               "/Caractère, 5 modalités/",
                               "/Caractère, 30 modalités/"),
                    DESCRIPTION=
                        c("Code de la géologie",
                          "Notation géologie",
                          "Description géologie", "Type superficiel",
                          "Colluvions, Eboulis, etc.",
                          "Type de formation", "Nature Géologique",
                          "Faciès des couches", "Age de la couche",
                          "Céno ou Méso", "Age autre", "Litho",
                          "Dureté", "Environnement", "Géochimie",
                          "Litho détaillée")))
#+end_src
...
|------------+---------------------------+---------------------------|
| CODE       | /Caractère, 31 modalités/ | Code de la géologie       |
| NOTATION   | /Caractère, 31 modalités/ | Notation géologie         |
| DESCR      | /Caractère, 31 modalités/ | Description géologie      |
| TYPEGEOL   | /Caractère, 4 modalités/  | Type superficiel          |
| APLOCALE   | /Caractère, 28 modalités/ | Colluvions, Eboulis, etc. |
| TYPEAP     | /Caractère, 7 modalités/  | Type de formation         |
| GEOLNAT    | /Caractère, 3 modalités/  | Nature Géologique         |
| ISOPIQUE   | /Caractère, 4 modalités/  | Faciès des couches        |
| AGEDEB     | /Caractère, 24 modalités/ | Age de la couche          |
| ERADEB     | /Caractère, 2 modalités/  | Céno ou Méso              |
| SYSDEB     | /Caractère, 5 modalités/  | Age autre                 |
| LITHOLOGIE | /Caractère, 16 modalités/ | Litho                     |
| DURETE     | /Caractère, 3 modalités/  | Dureté                    |
| ENVIRONMT  | /Caractère, 9 modalités/  | Environnement             |
| GEOCHIMIE  | /Caractère, 5 modalités/  | Géochimie                 |
| LITHOCOM   | /Caractère, 30 modalités/ | Litho détaillée           |
...
*** Enrichissement de la pédologie
    ...
    nationale DoneSol.  La localisation des types de sol s'opère par
    les Unités Cartographiques de Sols, appelées aussi Pédopaysages
    qui regroupent de zones homogènes qui contiennent néanmoins
    ...
    des unités cartographiques que l'on enrichie avec des variables de
    description des sols. Les intitulés des UCS sont obtenus par un
    travail manuel reporté à l'annexe 2 (par le site
    https://bourgogne.websol.fr/carto).  On peut citer ma thèse.
...
#+end_example
    ...
    effectivement.  Les valeurs manquantes correspondent /a priori/
    aux espaces urbanisés (mais cela ne se retrouve pas vraiment à
    partir du MOS).

#+begin_src R :exports results :results value :colnames yes :rownames no
(labv <- data.frame(NOM= c("NOUC", "SURFUC", "TARG", "TSAB", "TLIM",
                           "TEXTAG", "EPAIS", "TEG", "TMO", "RUE",
                           "RUD", "OCCUP", "DESCRp"),
                    TYPE= c("/Caractère de taille 2/",
                            "/Numérique en mètres carrés/",
                            "/Numérique en pourcentage/",
                            "/Numérique en pourcentage/",
                            "/Numérique en pourcentage/",
                            "/Caractère avec 9 modalités/",
                            "/Numérique en centimètres/",
                            "/Numérique en pour mille/",
                            "/Numérique en pourcentage/",
                            "/Numérique en millimètres/",
                            "/Numérique en millimètres/",
                            "/Numérique en pourcentage/",
                            "/Caractère avec 33 modalités/"),
                    DESCRIPTION=
                        c("Numéro de l'unité cartographique", 
                          ...
                          "Réserve Utile par défaut",
                          "Part de l'unité typologique",
                          "Description de la classe pédologique")))
#+end_src

#+RESULTS:
| NOM    | TYPE                          | DESCRIPTION                               |
|--------+-------------------------------+-------------------------------------------|
| NOUC   | /Caractère de taille 2/       | Numéro de l'unité cartographique          |
| SURFUC | /Numérique en mètres carrés/  | Surface de l'unité cartographique         |
| TARG   | /Numérique en pourcentage/    | Taux d'argile dans l'unité cartographique |
| TSAB   | /Numérique en pourcentage/    | Taux de sable dans l'unité cartographique |
| TLIM   | /Numérique en pourcentage/    | Taux de limon dans l'unité cartographique |
| TEXTAG | /Caractère avec 9 modalités/  | Classes de textures agrégées              |
| EPAIS  | /Numérique en centimètres/    | Épaisseur des sols                        |
| TEG    | /Numérique en pour mille/     | Taux d'éléments grossiers                 |
| TMO    | /Numérique en pourcentage/    | Taux de Matière organique                 |
| RUE    | /Numérique en millimètres/    | Réserve Utile par excès                   |
| RUD    | /Numérique en millimètres/    | Réserve Utile par défaut                  |
| OCCUP  | /Numérique en pourcentage/    | Part de l'unité typologique               |
| DESCRp | /Caractère avec 33 modalités/ | Description de la classe pédologique      |

*** Enrichissement des AOC de 1936

    Un travail préalable a été effectué sur les AOC de 1936 afin de
    compiler les différentes années de 1936 à 1940.  La localisation
    est effectuée par le centroïde des parcelles cadastrales car la
    géométrie des polygones ne correspond pas parfaitement (à la fois
    par la numérisation et parce que le cadastre a changé).
...
*** Distribution des indications géographiques
...
load("Inter/GeoRas.Rda") ; library(lattice)
tmp <- unique(Geo.Ras$LIBCOM[order(Geo.Ras$YCHF, decreasing= TRUE)])
Geo.Ras$LIBCOM <- factor(Geo.Ras$LIBCOM, levels= tmp)
fig.dat <- aggregate(model.matrix(~0+ factor(Geo.Ras$AOC))*
                     Geo.Ras$AREA/ 1000, by= list(Geo.Ras$LIBCOM), sum)
names(fig.dat)<-c("LIBCOM", "RIEN", "BGOR", "BOUR", "VILL", "PCRU", "GCRU")
...
my.pal <- brewer.pal(n= 9, name = "BuPu")[ c(1, 2, 4, 6, 8, 9)]
barchart(LIBCOM~ RIEN+ BGOR+ BOUR+ VILL+ PCRU+ GCRU, xlim= c(-100, 32000),
         xlab="Parcelles sous appellation d'origine contrôlée (hectare)",
...
library(mgcv) 
gam <- gam(AOC~ s(DEM)+ s(SLOPE)+ s(RAYAT)
           + s(X, Y, k= 100)+ LIBCOM+ EXPO 
         , data= Reg.Ras, family= ocat(R= 5))

summary(gam)

gamm <- gam(AOC~ s(DEM)+ s(SLOPE)+ s(RAYAT)
           + s(X, Y, k= 100)+ LIBCOM+ EXPO+ GEOL+ PEDO
         , data= Reg.Ras, family= ocat(R= 5))

## environ 2 heures
gamm5 <- gam(AOC~ s(DEM)+ s(SLOPE)+ s(RAYAT)
             + s(X, Y, k= 500)+ LIBCOM+ EXPO+ GEOL+ PEDO
           , data= Reg.Ras, family= ocat(R= 5))
summary(gamm5)
plot(gamm5, scale= 0, page= 1, rug= TRUE)

system.time( ## 5.5 heures
gamm8 <- gam(AOC~ s(DEM)+ s(SLOPE)+ s(RAYAT)
             +s(X, Y, k= 700)+ LIBCOM+ EXPO+ GEOL+ PEDO
           , data= Reg.Ras, family= ocat(R= 5))
)
summary(gamm8)
anova(gamm8)
plot(gamm8, scale= 0, page= 1)
modMontr <- list("gamm5"= gamm5, "gamm8"= gamm8)
save(modMontr, file= "Inter/modMontr.Rda")

load("Inter/modMontr.Rda")
summary(modMontr$gamm5)
plot(modMontr$gamm5, scale= 0, page= 1, rug= TRUE)
...
plot(gam, scale= 0, page= 1)
gam1 <- gam(AOC~ s(DEM)+ s(SLOPE)+ s(RAYAT)
            +s(X, Y, k= 100)+ LIBCOM+ EXPO
            +TYPEGEOL+ TYPEAP+ GEOLNAT+ ERADEB+ DURETE+ GEOCHIMIE
          , data= Reg.Ras, family= ocat(R= 5))
summary(gam1)
plot(gam1, scale= 0, page= 1)
anova(gam1)


gam2 <- gam(AOC~ s(DEM)+ s(SLOPE)+ s(RAYAT)
            +s(X, Y, k= 100)+ LIBCOM+ EXPO+ s(DISTCHF)
            +GEOLNAT+ ISOPIQUE+ ERADEB+ DURETE+ GEOCHIMIE
            + TARG+ TLIM+ EPAIS+ TEG+ TMO+ RUE
          , data= Reg.Ras, family= ocat(R= 5))
summary(gam2)
plot(gam2, scale= 0, page= 1)
anova(gam2)

gam3 <- gam(AOC~ s(DEM)+ s(SLOPE)+ s(RAYAT)
            +s(X, Y, k= 200)+ LIBCOM+ EXPO+ s(DISTCHF)
            +GEOLNAT+ ISOPIQUE+ ERADEB+ DURETE+ GEOCHIMIE
            + TARG+ TLIM+ EPAIS+ TEG+ TMO+ RUE
          , data= Reg.Ras, family= ocat(R= 5))
summary(gam3)
plot(gam3, scale= 0, page= 1)
anova(gam3)

gam3a <- gam(AOC~ s(DEM)+ s(SLOPE)+ s(RAYAT)
            +s(X, Y, k= 200)+ LIBCOM+ EXPO+ s(DISTCHF)
            + DESCR+ DESCRp
          , data= Reg.Ras, family= ocat(R= 5))
summary(gam3a)
plot(gam3a, scale= 0, page= 1)
anova(gam3a)


system.time( ## 40 min
gam4 <- gam(AOC~ s(DEM)+ s(SLOPE)+ s(RAYAT)
            +s(X, Y, k= 300)+ LIBCOM+ EXPO+ s(DISTCHF)
            +GEOLNAT+ ISOPIQUE+ DURETE+ TARG+ TLIM+ EPAIS+ TMO+ RUE
          , data= Reg.Ras, family= ocat(R= 5))
)
summary(gam4)
plot(gam4, scale= 0, page= 1)
anova(gam4)

system.time( ## 46 min
gam5 <- gam(AOC~ s(DEM)+ s(SLOPE)+ s(RAYAT)
            +s(X, Y, k= 400)+ LIBCOM+ EXPO+ s(DISTCHF)
            +GEOLNAT+ DURETE+ TARG+ TLIM+ EPAIS+ TMO+ RUE
          , data= Reg.Ras, family= ocat(R= 5))
)
summary(gam5)
plot(gam5, scale= 0, page= 1)
anova(gam5)

system.time( ## 5 heures
gam6 <- gam(AOC~ s(DEM)+ s(SLOPE)+ s(RAYAT)
            +s(X, Y, k= 500)+ LIBCOM+ EXPO+ s(DISTCHF)
            +GEOLNAT+ DURETE+ TARG+ TLIM+ EPAIS+ TMO+ RUE
          , data= Reg.Ras, family= ocat(R= 5))
)
summary(gam6)
plot(gam6, scale= 0, page= 1)
anova(gam6)

system.time( ## 4 heures
gam7 <- gam(AOC~ s(DEM)+ s(SLOPE)+ s(RAYAT)
            +s(X, Y, k= 600)+ LIBCOM+ EXPO+ s(DISTCHF)
            +GEOLNAT+ DURETE+ TARG+ TLIM+ EPAIS+ TMO+ RUE
          , data= Reg.Ras, family= ocat(R= 5))
)
summary(gam7)
plot(gam7, scale= 0, page= 1)
anova(gam7)

system.time( ## 7.7 heures
gam8 <- gam(AOC~ s(DEM)+ s(SLOPE)+ s(RAYAT)
            +s(X, Y, k= 700)+ LIBCOM+ EXPO+ s(DISTCHF)
            +GEOLNAT+ DURETE+ TARG+ TLIM+ EPAIS+ TMO+ RUE
          , data= Reg.Ras, family= ocat(R= 5))
)
summary(gam8)
plot(gam8, scale= 0, page= 1)
anova(gam8)

newgam <- list(gam6= gam6, gam7= gam7, gam8= gam8)
save(newgam, file= "Inter/newgam.Rda")
...
tmp <- table(Geo.Ras$NOUC)< 2200
Geo.Ras$PEDO <- factor(
    ifelse(!Geo.Ras$NOUC %in% names(tmp[tmp]),
           as.character(Geo.Ras$NOUC), "0AREF"))
GAMONT <- predict(modMontr$gamm8, newdata= Geo.Ras@data, type= "terms")
Geo.Ras$GMONraw <- rowSums(GAMONT, na.rm= TRUE)
Geo.Ras$GMONcor <- mean(GAMONT[, 1], na.rm= T)+
    rowSums(GAMONT[, -1], na.rm= TRUE)

cor(Geo.Ras$G900cor, Geo.Ras$GMONcor)
cor(Geo.Ras$G900raw, Geo.Ras$GMONraw)

Geo.Aoc <- subset(Geo.Ras, PAOC== 1)
unini <- function(x) (x- min(x))/ (max(x)- min(x))
Geo.Aoc$G900raw <- round(unini(Geo.Aoc$G900raw)* 100, 2)
Geo.Aoc$G900cor <- round(unini(Geo.Aoc$G900cor)* 100, 2)
Geo.Aoc$GMONraw <- round(unini(Geo.Aoc$GMONraw)* 100, 2)
Geo.Aoc$GMONcor <- round(unini(Geo.Aoc$GMONcor)* 100, 2)
...
cc <- rbind(
    data.frame(AOC= revalue(factor(Geo.Aoc$AOC), revAOC),
               Score= Geo.Aoc$GMONraw,
               Pr= "Uncorrected: without communes fixed effects"),
    data.frame(AOC= revalue(factor(Geo.Aoc$AOC), revAOC),
               Score= Geo.Aoc$GMONcor,
               Pr= "Corrected: with communes fixed effects"))
...
#+begin_src R
gg <- aggregate(Geo.Aoc$G900raw, by= list(Geo.Aoc$AOClb), mean, na.rm= T)
hh <- aggregate(Geo.Aoc$G900cor, by= list(Geo.Aoc$AOClb), mean, na.rm= T)
head(gg[order(gg$x, decreasing= TRUE), ], n= 10)
head(hh[order(hh$x, decreasing= TRUE), ], n= 10)

gg <- aggregate(Geo.Aoc$GMONraw, by= list(Geo.Aoc$AOClb), mean, na.rm= T)
hh <- aggregate(Geo.Aoc$GMONcor, by= list(Geo.Aoc$AOClb), mean, na.rm= T)
head(gg[order(gg$x, decreasing= TRUE), ], n= 10)
head(hh[order(hh$x, decreasing= TRUE), ], n= 10)

gg[gg$Group.1== 'Romanée-Conti',]
#+end_src
...
names(NVA) <- 1: 5
library(plyr)
Geo.Aoc$NIVEAU <- revalue(factor(Geo.Aoc$AOC), NVA)
Geo.Aoc$NAME <- ifelse(Geo.Aoc$AOC== 5, as.character(Geo.Aoc$AOClb),
                ifelse(Geo.Aoc$AOC< 4, as.character(Geo.Aoc$LIEUDIT), NA))
for (i in 1: nrow(Geo.Aoc)){
    if (is.na(Geo.Aoc$NAME[ i])){
        Geo.Aoc$NAME[ i] <- substr(Geo.Aoc$AOClb[ i],
                                   regexpr(" cru+", Geo.Aoc$AOClb[ i],
                                           perl= T)+ 5,
                                   nchar(as.character(Geo.Aoc$AOClb[ i])))
    } else {(Geo.Aoc$NAME[ i])}
}
Geo.Aoc$Concat <- paste0(Geo.Aoc$AOC, Geo.Aoc$LIBCOM, Geo.Aoc$NAME)
tmp_geo <- gBuffer(Geo.Aoc, byid= TRUE, width= 0)    
Poly.tst <- unionSpatialPolygons(tmp_geo, Geo.Aoc$Concat)

Poly.tst$xx <- as.character(row.names(Poly.tst))
attrTb <- data.frame(xx= Geo.Aoc$Concat[ !duplicated(Geo.Aoc$Concat)],
                     NIVEAU= Geo.Aoc$NIVEAU[ !duplicated(Geo.Aoc$Concat)],
                     LIBCOM= Geo.Aoc$LIBCOM[ !duplicated(Geo.Aoc$Concat)],
                     NAME=   Geo.Aoc$NAME[ !duplicated(Geo.Aoc$Concat)])
Poly.tst <- merge(Poly.tst, attrTb, by= "xx")

Poly.tst$PrdRaw <- aggregate(Geo.Aoc$G900raw,
                             by= list(Geo.Aoc$Concat), mean)[, 2]
Poly.tst$PrdCor <- aggregate(Geo.Aoc$G900cor,
                             by= list(Geo.Aoc$Concat), mean)[, 2]
Poly.tst$Area <- aggregate(Geo.Aoc$AREA,
                           by= list(Geo.Aoc$Concat), sum)[, 2]

Poly.tst@data[, 5: 7] <- round(Poly.tst@data[, 5: 7], 2)

library(RColorBrewer)
AocPal <- brewer.pal(5, "BuPu")
library(mapview)

mapviewOptions(basemaps = mapviewGetOption("basemaps")[ c(4, 1: 3, 5)])
tst <- mapview(Poly.tst,
               zcol= "NIVEAU", alpha.regions= .5,
               col.regions = AocPal,
               color= "white", legend.opacity= .5,
               label= Poly.tst$NAME, layerId= Poly.tst$xx,
               popup = popupTable(Poly.tst@data, feature.id= FALSE,
                                  zcol = c("NIVEAU", "LIBCOM", "NAME",
                                           "Area", "PrdRaw", "PrdCor")))
## addLogo(tst, "http://www7.inra.fr/fournisseurs/images/logo.jpg",
