...
tst <- mapview(Poly.Lxd,
               ...
               color= "white",
               label= Poly.Lxd$AOCLD, layerId = Poly.Lxd$AOCLD,
               popup = popupTable(Poly.Lxd@data,
                                  zcol = c("AOCLD", "Area", "AOC",
                                           "PrdRaw", "PrdCor")))

length(tst@map$x$calls[[ 6]]$args[[1]])
tst@map$x$calls[[ 6]]$args[[1]][[1]][[ 1]][[ 1]]

attr(tst@map$x$calls[[ 6]]$args[[1]], "dimnames")

tst@map$x$calls[[ 6]]$args[[1]][[1]][[ 1]][[ 1]] <-
    cbind(tst@map$x$calls[[ 6]]$args[[1]][[1]][[ 1]][[ 1]],
          layerId= "yop")
