...
*** The Informational Content of Geographical Indications
    ...
    R code and Shiny application to predict vineyard quality from an
    econometric model of geographical indications in /Côte d'Or/
    (Burgundy, France).  Relating the fine-scale biophysical
    attributes of vineyards to the hierarchical scheme of
    /Appellations d'Origine Contrôlée/ allows to predict a continuous
    quality score for each of the 60000 vineyards plots.  The Figure
    below reports this distribution according to the actual
    classification and a correction of preferential treatments betwwen
    /communes/ (see the documentations below for details).
...
Poly.Denom$PrdRaw <- aggregate(Geo.Denom$G900raw,
                               by= list(Geo.Denom$AOClb), mean)[, 2]
Poly.Denom$PrdCor <- aggregate(Geo.Denom$G900cor,
                               by= list(Geo.Denom$AOClb), mean)[, 2]
...
Poly.Appel$PrdRaw <- aggregate(Geo.Appel$G900raw,
                               by= list(Geo.Appel$LIEUDIT), mean)[, 2]
Poly.Appel$PrdCor <- aggregate(Geo.Appel$G900cor,
                               by= list(Geo.Appel$LIEUDIT), mean)[, 2]
