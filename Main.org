...
**** Text 1                                  :noheading:
     ...
     effects to compute the ordinal superiority measures reported in
     \autoref{Fig:2}.
...
**** Text 2                                  :noheading:
     ...
     different from the average /commune/ of the area in terms of the
     probability to be designated along the vertical dimension of GIs.
     /Communes/ from the /Côte de Nuits/ at the North of the region
     under study are more advantaged on average, as 10 /communes/ from
     this part are among the 15 most advantaged.  The proximity to
     Dijon where trials about the use of vineyard names take place
     between 1860 and 1936 is one potential explanation for this
     result.  The /communes/ that have a /syndicat/ (a group of wine
     producers) engaged in collective action appear to be privileged
     while the separation is not
     clear-cut.@@latex:\footnote{\cite{Jacq09} (p.189, 211) reports
     the \emph{communes} of \emph{Gevrey-Chambertin},
     ...
     advantaged /communes/ are not reputed (/Pernand-Vergeless/ and
     /Chorey-les-Beaune/) and some reputed /communes/ are
     disadvantaged (/Pommard/ and /Meursault/).  We find the ordinal
     superiority measures are only weakly positively correlated with
     average levels of actual GIs (R^2= 0.08, see \autoref{Fig:A2b}).

*** Informational content of GIs
**** Text 1                                  :noheading:
    
     \autoref{Tab:2} reports the decomposition from equations (5) to
     (8) with $q(X_i^*)$ predicted from the five OGAMs reported in
     \autoref{Tab:1} with increasing complexity order of spatial
     coordinates (the formulas used to compute the empirical
     counterparts of the decomposition are reported in RM Annex 1B,
     jointly with the R code to implement them).  As expected, the
     total signal reported in the first row of \autoref{Tab:2} is
     increasing and the total noise is decreasing.@@latex:\footnote{As
     ...
     decrease in total noise are two sides of the same coin, they show
     the increase in the variance of the latent variable predicted
     from tangible variables.}@@ In contrast to this monotonic
     relation between the signal and the complexity of the models, the
     results from joint, vertical and horizontal decomposition are
     more stationary between models.  As reported in the abstract, the
     average joint signal of about 77% is 4 times higher than the
     average joint noise of about 18%.  The vertical dimension has a
     higher informational content than the horizontal dimension, with
     an average signal to noise ratio of 1.37 ($=55/40$) compared to
     0.41 ($=29/70$).  The horizontal residual terms, which represents
     the marginal informational content of the vertical dimension once
     the horizontal dimension is fully taken into account, is higher
     than the full horizontal signal from using only the horizontal
     dimension.  The information about vineyard quality is more
     clearly assessed by the horizontal dimension of GIs. From the
     vertical residual terms, we show that the vertical dimension of
     GIs has about 20 % of bias in conveying information about
     vineyard quality, while this bias can be assessed by consumers
...
**** Text 1                                  :noheading:
     ...
     We run the same econometric analysis on the GIs of 1936, the year
     of the creation the national institute for GI management (INAO).
     At this period, the vertical dimension of GIs counts only three
     levels as reported in \autoref{Tab:SD} in Appendix: /Régional/ <
     /Village/ < /Grand Cru/ with respectively 57%, 40% and 3% of
     ...
     \emph{Saint-Romain} from the sample as they contains only one
     vertical level in 1936, soq their fixed effects are not
     identifiable.}@@ The table of joint significance, the marginal
     effects and the spatial smooth effects are reported in
     \autoref{Tab:A3}, \autoref{Fig:A4} and \autoref{Fig:A5} in
     Appendix.  The control for omitted /terroir/ variables is reached
     for smaller effective degrees of freedom (bootstrapped
     F-statistics for the omitted /terroir/ variable are reported in
     the bottom of \autoref{Tab:A3}, the violin plot is reported in
     the RM).  The hierarchy of the joint significance of RHS
     ...
     the 20th century.  Consequently, the horizontal dimension had a
     higher informational content than the vertical dimension, as it
     can be assessed from the first column of \autoref{Tab:3}.  The
     vertical dimension from GIs of 1936 had an average signal to
     noise ratio of 0.22 ($=17.8/79.7$) compared to 0.42 ($=29/68$)
     for the horizontal dimension (because the /commune/ delineations
     have not changed since 1936, the informational content of the
     horizontal dimension does not change).  The lower informational
     level of the vertical dimension of the GIs of 1936 is not
     exclusively dependant to the importance of /communes/ in
     delineation, a part of the loss is also attributable to the lower
     number of items, three instead of five in actual GIs.  Both
     effects lead to a smaller joint informational content of GIs with
     a signal to noise ratio of only 0.8 ($= 43.3/54.3$) that
     indicates significant improvements of the informational content
     of GIs in the last century.
...
**** Text 2                                  :noheading:
     ...
     With the predicted tangible vineyard quality from model ( V ) of
     \autoref{Tab:1}, we evaluate the informational content of
     simulated GI designations schemes as reported in \autoref{Tab:3}.
     We study eight counter-factual vertical dimensions of alternative
     schemes by changing the underlying latent score of vineyard (in
     S.0, S.I, S.II and S.III) and by changing the number of vertical
     items in schemes S.IV, S.V and S.VI.  We do not consider changing
     the horizontal dimension of GIs as changing administrative
     boundaries to increase the informational content of wine-related
     GIs is unlikely.  The detailed formula to simulate the
     alternative GI designation schemes are reported in RM.  The
     scheme S.0 is a benchmark that reproduce actual GI designations,
     including designation noise from $\xi$ by using surrogate
     residuals and estimated thresholds with /commune/ fixed effects.
     Then, we drop only designation errors in S.I, only the intangible
     /commune/ effects in S.II and both designation errors and
     /commune/ effects in S.IV.  The three last designation schemes
     simulate an increase in the number of vertical items from five to
     six.  In S.V, the /Bourgogne/ level is divided in two levels
     according to the mean of the latent variable used in S.0 for
     vineyards actually in /Bourgogne/.  The same is done for
     /Village/ and /Premier Cru/ in S.VI and S.VII.
